function NamesPanel(_owner, _l, _t, _w, _h, _observerproc, _closeproc) {
  ...
  me.setbounds = function (l, t, w, h) {
    ...
    cb.bnds(
      w - cb_width - cb_margin,
      (tl_height - cb_width) / 2,
      cb_width,
      cb_width,
    );
    md.bnds(
      w - mod_width - 2 * mod_margin,
      tl_height,
      mod_width + 2 * mod_margin,
      h - tl_height - tb_height,
    );
    kb.bnds(
      0,
      tl_height,
  ...
  var dragmove = function (ev) {
    ...
    me.setbounds(left + coords.x - xx, top + coords.y - yy, width, height);
    xx = coords.x;
    yy = coords.y;
  };
  ...
  var resizemove = function (ev) {
    ev.preventDefault();
    var coords = getPageCoords(ev);
    var w = width + coords.x - xx < 100 ? width : width + coords.x - xx;
  ...
  wp.stls(
    "position:absolute;z-index:9000;border-bottom-left-radius:10px;border-bottom-right-radius:10px;overflow:hidden;border: 1px solid #b4b4b4;transition:transform 0.2s linear;transform:scale(0)",
  );
  tl.stls("background-color:rgba(210,210,210,1);position:absolute");
  cb.stls(
    "background-color:rgba(0,0,0,0);position:absolute;background-position:center;background-repeat:no-repeat;background-size:100% 100%",
  );
  ...
  var showCurrentKey = function () {
    ...
    for (var i = current_key; i < keys.length; i++) {
      if (nmes.indexOf(keys[i].key) === -1) {
        current_key = i;
        keys[i].stls(
          "background-color:rgba(50,50,50,1);color:rgba(230,230,230,1)",
        );
        return;
      }
    }
  ...
  me.refreshkeyboard = function () {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (nmes.indexOf(keys[i].key) != -1) {
        t.rmevt();
        t.stls(
          "background-color:rgba(200,200,200,1);color:rgba(150,150,150,1)",
        );
      } else {
        seton(i);
        // t.stls("background-color:rgba(200,200,200,1);color:rgba(30,30,30,1)");
  ...
  var createkey = function (_s, _x, _y) {
    ...
    t.stls(
      "border: 1px solid #b4b4b4;background-color:rgba(200,200,200,1);color:rgba(30,30,30,1);position:absolute;border-radius:5px;width:" +
        key_width +
        "px;height:" +
        key_height +
        "px;font-size: 18px;font-weight:normal;font-family: Helvetica, Arial, sans-serif;text-align: center;white-space: pre-wrap;margin: 0px;line-height:" +
        key_height +
        "px;vertical-align:middle;top:" +
        _y +
        "px;left:" +
        _x +
        "px",
  ...
  var initkeyboard = function () {
    ...
    for (var i = 0; i < kbd.length; i++) {
      ...
      for (var j = 0; j < kbd[i].length; j++) {
        createkey(
          kbd[i][j] + m,
          hgap + j * (key_width + hgap),
          vgap + i * (key_height + vgap),
        );
      }
    }
    current_key = 0;
  ...
  var select_mod = function (_s) {
    ...
    for (var i = 0; i < mods.length; i++) {
      mods[i].stls(
        "background-color:rgba(90,90,90,1);color:rgba(230,230,230,1)",
      );
    }
    mods[_s].stls(
      "background-color:rgba(200,200,200,1);color:rgba(30,30,30,1)",
    );
    initkeyboard();
  };
  ...
  var createmod = function (_c, _m) {
    ...
    t.stls(
      "border: 1px solid #b4b4b4;background-color:rgba(90,90,90,1);color:rgba(230,230,230,1);position:absolute;border-radius:5px;width:" +
        mod_width +
        "px;height:" +
        mod_height +
        "px;font-size: 18px;font-weight:bold;font-family: Helvetica, Arial, sans-serif;text-align: center;white-space: pre-wrap;margin: 0px;line-height:" +
        mod_height +
        "px;vertical-align:middle;top:" +
        (gap + i * (mod_height + gap)) +
        "px;left:" +
        mod_margin +
        "px",
    );
  ...
  var select_tab = function (_s) {
    if (_s !== current_tab) {
      ...
      tabs[_s].stls(
        "background-color:rgba(230,230,230,1);color:rgba(30,30,30,1)",
      );
  ...
  var createtab = function (_n, _c) {
    ...
    t.stls(
      "border-left: 1px solid #b4b4b4;border-top: 1px solid #b4b4b4;border-right: 1px solid #b4b4b4;background-color:rgba(90,90,90,1);color:rgba(230,230,230,1);position:absolute;border-top-right-radius:10px;width:" +
        tab_width +
        "px;height:" +
        tab_height +
        "px;font-size: 18px;font-weight:bold;font-family: Helvetica, Arial, sans-serif;text-align: center;white-space: pre-wrap;margin: 0px;line-height:" +
        tab_height +
        "px;vertical-align:middle;top:" +
        (tl_height - tab_height) +
        "px;left:" +
        (tab_left_margin + tabs.length * (tab_width + tab_gap)) +
        "px",
    );
    t.innerHTML = _n;
  ...
  createtab(
    "\u0394",
    "\u0391\u0392\u0393\u0394\u0395\u0396\u0397\u0398_\u0399\u039A\u039B\u039C\u039D\u039E\u039F\u03A0_\u03A1\u03A3\u03A4\u03A5\u03A6\u03A7\u03A8\u03A9",
  );
  createtab(
    "\u03B4",
    "\u03B1\u03B2\u03B3\u03B4\u03B5\u03B6\u03B7\u03B8\u03B9_\u03BA\u03BB\u03BC\u03BD\u03BE\u03BF\u03C0\u03C1_\u03C2\u03C3\u03C4\u03C5\u03C6\u03C7\u03C8\u03C9",
  );
  select_tab(0);
  ...
  wp.md(dragdown);
  wp.mu(dragup);
  wp.addEventListener("touchstart", dragdown, false);
