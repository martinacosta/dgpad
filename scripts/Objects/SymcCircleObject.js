...
function SymcCircleObject(_construction, _name, _P, _C) {
  ...
  var simcPuntos=_construction.getAllObjectsFromType("point");
	  for (var j = 0, len2 = simcPuntos.length; j < len2; j++) {
		  ...
		  if (simcPuntos[j].isChild(P)&&simcPuntos[j].isChild(M)){ptoExistente=j}
		  
	  }
	  if (ptoExistente>-1){var N=simcPuntos[ptoExistente]}
	  if (ptoExistente==-1) {var N = new SymcPointObject(_construction, "_P", P, M);_construction.add(N);}
