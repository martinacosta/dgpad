...
function HomoLineObject(_construction, _name, _E, _R, _P) {
  ...
  var ptoExistente=-1;
	  for (var j = 0, len2 = homoPuntos.length; j < len2; j++) {
		  ...
		  if (homoPuntos[j].isChild(E)&&homoPuntos[j].isChild(pun1)){ptoExistente=j}
		  
	  }
	  if (ptoExistente>-1){var trans1=homoPuntos[ptoExistente]}
	  if (ptoExistente==-1) {var trans1 = new HomoPointObject(_construction, "_P", E, pun1, P);_construction.add(trans1);}
