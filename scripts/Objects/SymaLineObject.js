...
function SymaLineObject(_construction, _name, _L, _R) {
  ...
  var simaPuntos=_construction.getAllObjectsFromType("point");
	  for (var j = 0, len2 = simaPuntos.length; j < len2; j++) {
		  ...
		  if (simaPuntos[j].isChild(L)&&simaPuntos[j].isChild(pun1)){ptoExistente=j}
		  
	  }
	  if (ptoExistente>-1){var ref1=simaPuntos[ptoExistente]}
	  if (ptoExistente==-1) {var ref1 = new SymaPointObject(_construction, "_P", L, pun1);_construction.add(ref1);}
