...
function SymcLineObject(_construction, _name, _P, _R) {
	...
	var simcPuntos=_construction.getAllObjectsFromType("point");
	  for (var j = 0, len2 = simcPuntos.length; j < len2; j++) {
		  ...
		  if (simcPuntos[j].isChild(P)&&simcPuntos[j].isChild(pun1)){ptoExistente=j}
		  
	  }
	  if (ptoExistente>-1){var trans1=simcPuntos[ptoExistente]}
	  if (ptoExistente==-1) {var trans1 = new SymcPointObject(_construction, "_P", P, pun1);_construction.add(trans1);}
