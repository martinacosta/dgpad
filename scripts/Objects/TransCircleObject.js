...
function TransCircleObject(_construction, _name, _V, _C) {
  ...
  var trasPuntos=_construction.getAllObjectsFromType("point");
	  for (var j = 0, len2 = trasPuntos.length; j < len2; j++) {
		  ...
		  if (trasPuntos[j].isChild(V)&&trasPuntos[j].isChild(M)){ptoExistente=j}
		  
	  }
	  if (ptoExistente>-1){var N=trasPuntos[ptoExistente]}
	  if (ptoExistente==-1) {var N = new TransPointObject(_construction, "_P", V, M);_construction.add(N);}
