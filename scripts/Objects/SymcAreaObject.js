...
function SymcAreaObject(_construction, _name, _V, _P) {
  ...
  var simcPuntos=Cn.getAllObjectsFromType("point");
  ...
  for (var i = 0, len = puntos.length; i < len; i++) {
	  ...
	  for (var j = 0, len2 = simcPuntos.length; j < len2; j++) {
		  ...
		  if (simcPuntos[j].isChild(V)&&simcPuntos[j].isChild(P.getPt(i))){ptoExistente=j}
		  
	  }
	  if (ptoExistente>-1){var vertice=simcPuntos[ptoExistente]}
	  if (ptoExistente==-1) {var vertice = new SymcPointObject(_construction, "_P", V, P.getPt(i));_construction.add(vertice);}
	  polRef.push(vertice);
