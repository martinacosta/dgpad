...
function SymaCircleObject(_construction, _name, _L, _P) {
  ...
  var simaPuntos=_construction.getAllObjectsFromType("point");
	  for (var j = 0, len2 = simaPuntos.length; j < len2; j++) {
		  ...
		  if (simaPuntos[j].isChild(L)&&simaPuntos[j].isChild(M)){ptoExistente=j}
		  
	  }
	  if (ptoExistente>-1){var N=simaPuntos[ptoExistente]}
	  if (ptoExistente==-1) {var N = new SymaPointObject(_construction, "_P", L, M);_construction.add(N);}
