...
function RotationAreaObject(_construction, _name, _E, _pol, _P) {
  ...
  var rotPuntos=Cn.getAllObjectsFromType("point");
  
  
  for (var i = 0, len = puntos.length; i < len; i++) {
	  var ptoExistente=-1;
	  for (var j = 0, len2 = rotPuntos.length; j < len2; j++) {
		  
		  if (rotPuntos[j].isChild(E)&&rotPuntos[j].isChild(pol.getPt(i))){ptoExistente=j}
		  
	  }
	  if (ptoExistente>-1){var vertice=rotPuntos[ptoExistente]}
	  if (ptoExistente==-1) {var vertice = new RotationPointObject(_construction, "_P", E, pol.getPt(i),P);_construction.add(vertice);}
	  polRef.push(vertice);
