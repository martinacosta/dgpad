function FixedAngleConstructor() {
    ...
    this.newObj = function(_zc, _C) {
        ...
        var obj = new FixedAngleObject(_zc.getConstruction(), "_af", _C[0], trig);
        obj.setExp(angle);
        return (obj);
    };
    ...
    this.preview = function(ev, zc) {
        ...
        /* switch (len) {
            ...
            case 1: */
					...
					xA0 = c[0].P2.getX();
					yA0 = c[0].P2.getY();
					var t0 = $U.computeBorderPoints(xM, yM, xA0 - xM, yA0 - yM, zc.getWidth(), zc.getHeight());
                    xA = zc.mouseX(ev);
                    yA = zc.mouseY(ev);
                    var t = $U.computeAngleParams(c[0].P2.getX(), c[0].P2.getY(), c[0].P1.getX(), c[0].P1.getY(), zc.mouseX(ev), zc.mouseY(ev));
                    fromA = t.startAngle;
                    toA = t.endAngle;
                    trig = t.Trigo;
				...
				ctx.beginPath();
				ctx.moveTo(c[0].P1.getX(), c[0].P1.getY());
				ctx.lineTo(t0[2], t0[3]);
				ctx.stroke();
				ctx.closePath();
                //break;
        //}
        ...
        if (!isNaN(xA)) {
            ...
            var a = Math.sqrt((xM - c[0].P1.getX()) * (xM - c[0].P1.getX()) + (yM - c[0].P1.getY()) * (yM - c[0].P1.getY()));
            var k = b / (a + b);
            var x = xA + k * (c[0].P1.getX() - xA) - xM;
            var y = yA + k * (c[0].P1.getY() - yA) - yM;
		...
		/* switch (len) {
            case 1:
			   ...
			   ctx.font = "16px Verdana";
                ...
                ctx.fillText(this.getC(0).getName(),this.getC(0).P2.getX()+20,this.getC(0).P2.getY());
                break;
            case 2: */
				var display = AOC180;
				display = display * 180 / Math.PI;
