...
function PropertiesPanel(_canvas) {
  ...
  me.getDocObject().addEventListener('touchstart', dragdown, false);
	...
	me.setMonkeyMode = function(_val) {
        canvas.magnifyManager.setMonkeyMode(_val);
    };
    me.getMonkeyMode = function() {
        return canvas.magnifyManager.getMonkeyMode();
    };
...
function props_messagePanel(_owner) {
	...
	var MONKEYcallback = function(val) {
        _owner.setMonkeyMode(val);
    };
	...
	var cbMonkey = new Checkbox(me.getDocObject(), 10, ch, 200, 30, _owner.getMonkeyMode(), $L.props_general_monkey, MONKEYcallback);
    cbMonkey.setTextColor("#252525");
    ch += 30;
